...
const DEFAULT_CONFIG = {
  ...
  async function testServerConnection() {
    try {
      ...
      let response = await fetch(`${currentConfig.apiUrl}/health`, {
        method: "GET",
      ...
      if (response.status === 404) {
        ...
        response = await fetch(`${currentConfig.apiUrl}`, {
          method: "GET",
        })
      }
      ...
      // Nếu server trả về 404, vẫn coi là kết nối được
      ...
      return {
        ...
        message: isConnected ? "Kết nối thành công" : `Kết nối thất bại (HTTP ${response.status})`,
      }
    } catch (error) {
