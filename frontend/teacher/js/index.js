...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.setQuestionEditMode = (questionDiv) => {
    ...
    <div class="one-answer-form">
      ...
      <input type="text" id="question" name="question" required value="${questionData.content[0] || ""}">
        ...
        <div id="options-list">
          ${questionData.content
            ...
            <div class="option-item">
              <input type="text" name="option" required value="${option}">
              <input type="radio" name="correctAnswer" value="${idx}" ${option === questionData.correctAnswers ? "checked" : ""}>
      ...
      <div class="options-container">
        ...
        <div id="options-list">
          ${questionData.content
            ...
            <div class="option-item">
              <input type="text" name="option" required value="${option}">
              <input type="checkbox" name="correctAnswer" value="${idx}" ${Array.isArray(questionData.correctAnswers) && questionData.correctAnswers.includes(option) ? "checked" : ""}>
              <button type="button" class="remove-option-btn"><i class="fas fa-times"></i></button>
            </div>
          `,
            )
  ...
  `
            break
          case "Ghép nối":
            // Xác định điểm giữa để tách mục và ghép nối
            const midPoint = Math.ceil(questionData.content.length / 2)
            const items = questionData.content.slice(1, midPoint)
            const matches = questionData.content.slice(midPoint)

            formHTML = `
<div class="matching-form">
  <div class="form-group">
    <label for="title">Tiêu đề bài ghép nối:</label>
    <input type="text" id="title" name="title" required value="${questionData.content[0] || ""}" placeholder="Ví dụ: Ghép nối người với công việc">
  </div>
      ...
      <div id="items-list">
        ${items
          .map(
            (item, idx) => `
          <div class="item-row">
            <input type="text" name="item" required value="${item}" placeholder="Câu hỏi ${idx + 1}">
      ...
      <div id="matches-list">
        ${matches
          .map(
            (match, idx) => `
          <div class="match-row">
            <input type="text" name="match" required value="${match}" placeholder="Từ khóa nối ${idx + 1}">
...
<div class="plan-map-diagram-form">
  <div class="form-group">
    ...
    <select id="type" name="type" required onchange="updatePlanMapDiagramForm(this)">
      <option value="map" ${questionData.content[0] === "map" ? "selected" : ""}>Ghi nhãn Bản đồ (Chọn từ A-H)</option>
      <option value="ship" ${questionData.content[0] === "ship" ? "selected" : ""}>Sơ đồ Tàu (Nhập đáp án)</option>
    </select>
  ...
  <div class="form-group">
    ...
    <textarea id="instructions" name="instructions" rows="3" required placeholder="Nhập hướng dẫn cho câu hỏi">${questionData.content[1] || ""}</textarea>
  </div>
  
  <div class="form-group">
    <label for="image">Hình ảnh hiện tại:</label>
    <div id="imagePreview" class="image-preview">
      <img src="${questionData.content[2] || ""}" alt="Hình ảnh" style="max-width: 200px; max-height: 200px;">
    </div>
    <label for="image">Tải lên hình ảnh mới (nếu cần):</label>
    ...
    ${questionData.content
      ...
      <div class="label-row">
        <div class="label-input-group">
          <label for="label${idx + 1}">Nhãn ${idx + 1}:</label>
          <input type="text" id="label${idx + 1}" name="label" required value="${label}" placeholder="Nhập nhãn">
        </div>
        <div class="answer-input-group map-answer-group" ${questionData.content[0] !== "map" ? 'style="display: none;"' : ""}>
          <label for="answer${idx + 1}">Đáp án:</label>
          <select id="answer${idx + 1}" name="answer" required>
            <option value="">-- Chọn --</option>
            ${["A", "B", "C", "D", "E", "F", "G", "H"]
              .map(
                (letter) =>
                  `<option value="${letter}" ${questionData.correctAnswers[idx] === letter ? "selected" : ""}>${letter}</option>`,
              )
              .join("")}
          </select>
        </div>
        <div class="answer-input-group ship-answer-group" ${questionData.content[0] !== "ship" ? 'style="display: none;"' : ""}>
          <label for="shipAnswer${idx + 1}">Đáp án:</label>
          <input type="text" id="shipAnswer${idx + 1}" name="shipAnswer" required value="${questionData.correctAnswers[idx] || ""}" placeholder="Nhập đáp án">
        </div>
...
<div class="note-completion-form">
  <label for="instructions">Hướng dẫn:</label>
  <input type="text" id="instructions" name="instructions" value="${questionData.content[0] || "Hoàn thành ghi chú. Viết MỘT TỪ VÀ/HOẶC MỘT SỐ vào mỗi khoảng trống."}" required>
  <label for="topic">Chủ đề:</label>
  <input type="text" id="topic" name="topic" value="${questionData.content[1] || ""}" required>
  <div id="notes-container">
    ${questionData.content
      ...
      .map(
        (note, idx) => `
      <div class="note-row">
        <label>Ghi chú ${idx + 1} (sử dụng [ANSWER] cho chỗ trống):</label>
    ...
    <div id="note-answers-list">
      ${questionData.correctAnswers
        ...
        <div class="answer-row">
          <span class="answer-label">Đáp án ${idx + 1}:</span>
...
<div class="form-table-completion-form">
  <label for="instructions">Hướng dẫn:</label>
  <input type="text" id="instructions" name="instructions" value="${questionData.content[0] || "Hoàn thành bảng. Viết KHÔNG QUÁ MỘT TỪ VÀ/HOẶC MỘT SỐ cho mỗi khoảng trống."}" required>
  <table id="formTable">
    <thead>
      <tr>
        <th>Cột 1</th>
    ...
    <tbody>
      ${(() => {
        let rows = ""
        for (let i = 1; i < questionData.content.length; i += 3) {
          rows += `
          <tr>
            <td><input type="text" name="cell" value="${questionData.content[i] || ""}" required></td>
            <td><input type="text" name="cell" value="${questionData.content[i + 1] || ""}" required></td>
            <td><input type="text" name="cell" value="${questionData.content[i + 2] || ""}" required></td>
            <td><input type="text" name="tableAnswer" value="${questionData.correctAnswers[Math.floor((i - 1) / 3)] || ""}" required></td>
...
`
          ...
          case "Hoàn thành lưu đồ":
            // Xác định điểm giữa để tách mục và tùy chọn
            const flowItemCount = Math.floor((questionData.content.length - 2) / 2)
            const flowItems = questionData.content.slice(2, 2 + flowItemCount)
            const flowOptions = questionData.content.slice(2 + flowItemCount)

            formHTML = `
<div class="flow-chart-completion-form">
  <label for="title">Tiêu đề:</label>
  <input type="text" id="title" name="title" value="${questionData.content[0] || ""}" required>
  <label for="instructions">Hướng dẫn:</label>
  <input type="text" id="instructions" name="instructions" value="${questionData.content[1] || ""}" required>
